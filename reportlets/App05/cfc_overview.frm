...
FROM "TRSDB"."V_TRS_FACT_CFC_TAX_INCOME_TOTAL" t
order by to_char(t.period,'yyyy-mm') desc]]></Query>
<PageQuery>
<![CDATA[]]></PageQuery>
</TableData>
...
, ACCOUNT_BALANCE AS(
SELECT 
	CONCAT(period, CONCAT('-',LPAD(t1.month, 2, '0'))) as ym,
	t1.country_id,
	t2.CURRENT_CODE,
	t1.period,
	t1.MONTH,
...
FROM TRSDB.MV_TRS_TB_ACTUAL_ACCOUNT_AMOUNT t1
...
AND t2.ENTITY_TYPE_ID = '003' /*此版MV_TB只有002*/
	...
	${if(LEN(P_PERIOD)=0,"","AND (CONCAT(period, CONCAT('-',LPAD(t1.month, 2, '0')))) IN ('"+P_PERIOD+"')")}
	GROUP BY T2_CODE,
	t2.CURRENT_CODE,
	t1.country_id,
	t1.period,
...
SELECT *
	...
	SUM(STANDARD_AMOUNT)
	FOR T2_CODE
	IN ('4000' AS CODE4000,'5000'AS CODE5000,'6000'AS CODE6000,'6500'AS CODE6500,'7000'AS CODE7000,'7950'AS CODE7950)
	)
)
,TABLE1 AS(
SELECT
	CODE4000,
	NVL(CODE5000,0) AS CODE5000,
	NVL(CODE6000,0) AS CODE6000,
	NVL(CODE6500,0) AS CODE6500,
	NVL(CODE7000,0) AS CODE7000,
	(CODE4000-NVL(CODE5000,0)-NVL(CODE6000,0)+NVL(CODE6500,0)+NVL(CODE7000,0)) AS TAX,
	NVL(CODE7950,0) AS CODE7950
FROM PIVOT_TABLE
)
SELECT
	nvl(CODE4000,0) AS CODE4000,  --營業收入
	nvl(CODE4000-CODE5000,0) AS CODE5900, --營業毛利
	nvl(CODE4000-CODE5000-CODE6000+CODE6500,0) AS CODE6900, --營業利潤
	nvl(CODE4000-CODE5000-CODE6000+CODE6500+CODE7000-CODE7950,0) AS CODE8200, --淨利潤
	nvl(CODE7950/TAX,0) AS tax_rates, --有效稅率
	CODE7950 --所得稅費用
FROM TABLE1]]></Query>
<PageQuery>
<![CDATA[]]></PageQuery>
...
, calc AS (
SELECT
	NVL(t1.CODE4000,0) as CODE4000,
	NVL(t1.CODE5000,0) AS CODE5000,
	NVL(t1.CODE6000,0) AS CODE6000,
	NVL(t1.CODE6500,0) AS CODE6500,
	NVL(t1.CODE7000,0) AS CODE7000,
	NVL(t1.CODE7950,0) AS CODE7950,
	NVL(t2.tax_payment,0) AS TAX_PAYMENT,
	NVL(CODE4000,0)-NVL(CODE5000,0)-NVL(CODE6000,0)+NVL(CODE6500,0)+NVL(CODE7000,0) AS TAX,
	NVL(t2.TAX_INC,0) AS TAX_INC,
	NVL(t1.CODE7950,0) + NVL(t2.tax_payment,0) AS INCLUDE_CFC 
FROM PIVOT_TABLE t1
FULL JOIN ALL_TAX_PAY t2 ON 1 = 1
)
SELECT a.*, NVL(a.INCLUDE_CFC/NULLIF(a.TAX,0),0) AS EFFECTIVE_TAX_RATE_CFC
FROM calc a]]></Query>
<PageQuery>
<![CDATA[]]></PageQuery>
</TableData>
...
<TableData name="Rep_ETR_CFC的副本" class="com.fr.data.impl.DBTableData">
<Desensitizations desensitizeOpen="false"/>
<Parameters>
<Parameter>
<Attributes name="GP_USERNAME"/>
...
SELECT 
	...
	t2.CURRENT_CODE AS INV_CURRENT_CODE,
	case when t1.TAX_INC <= 0 THEN 0 else t1.TAX_INC end as TAX_INC_ADJ,
	case when t1.TAX_INC <= 0 THEN 0 ELSE t4.tax_rate * t1.TAX_INC END AS TAX_PAYMENT,
	MONTHS_BETWEEN(TO_DATE('${P_PERIOD}', 'YYYY-MM'), t5.START_DATE) +1 AS BUSINESS_MONTH
FROM PROFIT_1 t1
LEFT JOIN v_trs_dim_entity_cur t2 ON t1.inv_entity_code = t2.entity_code and t2.entity_type_id = '003'
LEFT JOIN trs_dim_entity_i18n t3 ON t2.current_code = t3.entity_id
LEFT JOIN V_TRS_FACT_COUNTRY_TAX t4 on t1.COUNTRY_CODE = t4.COUNTRY_CODE
LEFT JOIN V_TRS_DIM_COMPANY t5 on t1.current_code = t5.current_code
WHERE t1.PERIOD BETWEEN t4.start_date AND t4.end_date AND t3."LANGUAGE" ='${fr_locale}'
${if(LEN(P_ENTITY)=0,"","AND t1.current_code IN ('"+P_ENTITY+"')")}
${if(LEN(P_CFC_NAME)=0,"","AND t2.current_code IN ('"+P_CFC_NAME+"')")}
${if(LEN(P_SCENARIO)=0,"","AND t1.scenario IN ('"+P_SCENARIO+"')")}
${if(LEN(P_PERIOD)=0,"","AND t1.YR_MONTH IN ('"+P_PERIOD+"')")}
...
<Widget class="com.fr.form.ui.container.WAbsoluteLayout$BoundsWidget">
<InnerWidget class="com.fr.form.ui.WidgetErrorMarker" plugin-version="8.27" oriClass="com.fr.plugin.form.widget.core.RHIframe" pluginID="com.fr.solution.plugin.form.widget.rh.iframe.v10">
<WidgetName name="rHIframe0"/>
<WidgetID widgetID="6c5398ec-8cf0-4b15-abb4-488c1158fbb5"/>
<WidgetAttr description="" aspectRatioLocked="false" aspectRatioBackup="-1.0">
<MobileBookMark oldWidgetName="" bookMarkName="rHIframe0" frozen="false" index="-1" useBookMark="false"/>
<PrivilegeControl/>
</WidgetAttr>
<Attr scrollY="false" method="POST" scrollX="false" contentAdapt="false" customUrlEncode="true"/>
<RHIframeAttr plugin-version="8.27" class="com.fr.plugin.form.widget.core.RHIframeAttr" pluginID="com.fr.solution.plugin.form.widget.rh.iframe.v10">
<RHIframeSource pluginID="com.fr.solution.plugin.form.widget.rh.iframe.v10" plugin-version="8.27" class="com.fr.plugin.form.widget.core.TemplateSource">
...
<UUID uuid="c56e3068-08d8-4002-b3fe-6f9f07079af3"/>
...
<FileAttrErrorMarker-TemplateCloudInfoAttrMark class="com.fr.base.io.FileAttrErrorMarker" plugin-version="3.25.0.20240306" oriClass="com.fr.plugin.cloud.analytics.attr.TemplateInfoAttrMark" pluginID="com.fr.plugin.cloud.analytics.v11">
<TemplateCloudInfoAttrMark createTime="1651036965697"/>
</FileAttrErrorMarker-TemplateCloudInfoAttrMark>
<FileAttrErrorMarker-I18NMap class="com.fr.base.io.FileAttrErrorMarker" plugin-version="2.1.6" oriClass="com.fr.plugin.i18n.bundle.configurator.data.I18NAttrMark" pluginID="com.fr.plugin.i18n.bundle.v11">
<Attributes default="" backup="en_US" languageType="0"/>
<I18N description="" key="MONTH">
<zh_TW>
<![CDATA[按月]]></zh_TW>
<en_US>
<![CDATA[Monthly]]></en_US>
...
<I18N description="" key="business_tax">
<zh_TW>
<![CDATA[營業稅]]></zh_TW>
<en_US>
<![CDATA[Business Tax]]></en_US>
</I18N>
<I18N description="" key="gross_profit">
<zh_TW>
<![CDATA[營業毛利]]></zh_TW>
<en_US>
<![CDATA[Gross Profit]]></en_US>
</I18N>
<I18N description="" key="mom">
<zh_TW>
<![CDATA[較上個月]]></zh_TW>
<en_US>
<![CDATA[MoM]]></en_US>
</I18N>
<I18N description="" key="effective_tax_rate_include_cfc">
<zh_TW>
...
<I18N description="" key="actual_payment">
<zh_TW>
<![CDATA[實繳金額]]></zh_TW>
<en_US>
<![CDATA[Tax Payment]]></en_US>
</I18N>
<I18N description="" key="period_paid_amount">
<zh_TW>
<![CDATA[當期金額]]></zh_TW>
<en_US>
<![CDATA[Current Amount]]></en_US>
</I18N>
<I18N description="" key="gross_profit_margin">
<zh_TW>
<![CDATA[毛利率]]></zh_TW>
<en_US>
<![CDATA[Gross Profit Margin]]></en_US>
</I18N>
<I18N description="查詢" key="search">
<zh_TW>
<![CDATA[查詢]]></zh_TW>
<en_US>
<![CDATA[Search]]></en_US>
</I18N>
<I18N description="" key="revenue">
<zh_TW>
<![CDATA[營業收入]]></zh_TW>
<en_US>
<![CDATA[Sales]]></en_US>
</I18N>
<I18N description="" key="QUARTER">
<zh_TW>
<![CDATA[按季]]></zh_TW>
<en_US>
<![CDATA[Quarterly]]></en_US>
</I18N>
<I18N description="" key="YEAR">
<zh_TW>
<![CDATA[按年]]></zh_TW>
<en_US>
<![CDATA[Annually]]></en_US>
</I18N>
<I18N description="" key="profit">
<zh_TW>
<![CDATA[營業利潤]]></zh_TW>
<en_US>
...
<I18N description="" key="billed_payment">
<zh_TW>
<![CDATA[帳列費用]]></zh_TW>
<en_US>
<![CDATA[Tax Expense]]></en_US>
</I18N>
<I18N description="" key="yoy">
<zh_TW>
<![CDATA[較去年同期]]></zh_TW>
<en_US>
<![CDATA[YoY]]></en_US>
</I18N>
<I18N description="" key="value_added_tax">
<zh_TW>
<![CDATA[增值稅]]></zh_TW>
<en_US>
...
<I18N description="" key="business_income_tax">
<zh_TW>
<![CDATA[企業所得稅]]></zh_TW>
<en_US>
<![CDATA[Corporate Income Tax]]></en_US>
</I18N>
<I18N description="" key="ytd_paid_amount">
<zh_TW>
<![CDATA[本年累計]]></zh_TW>
<en_US>
<![CDATA[Accumulated amount]]></en_US>
</I18N>
<I18N description="" key="profit_margin">
<zh_TW>
<![CDATA[利潤率]]></zh_TW>
<en_US>
<![CDATA[Operating Profit Margin]]></en_US>
</I18N>
<I18N description="" key="net_profit">
<zh_TW>
<![CDATA[淨利潤]]></zh_TW>
<en_US>
<![CDATA[Net Profit]]></en_US>
</I18N>
<I18N description="" key="by_company">
<zh_TW>
<![CDATA[各公司]]></zh_TW>
<en_US>
...
<StrategyConfig dsName="Dic_Scenario" enabled="true" useGlobal="false" shouldMonitor="true" shouldEvolve="false" scheduleBySchema="false" timeToLive="1500000" timeToIdle="86400000" updateInterval="1500000" terminalTime="" updateSchema="0 0 8 * * ? *" activeInitiation="false"/>
<StrategyConfig dsName="Rep_ETR_CFC的副本" enabled="false" useGlobal="true" shouldMonitor="true" shouldEvolve="false" scheduleBySchema="false" timeToLive="1500000" timeToIdle="86400000" updateInterval="1500000" terminalTime="" updateSchema="0 0 8 * * ? *" activeInitiation="false"/>
