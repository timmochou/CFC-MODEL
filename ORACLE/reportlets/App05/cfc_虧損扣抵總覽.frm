...
FROM "TRSDB"."V_TRS_CFC_10Y_DEDUCTION_RESULT" t
...
<![CDATA[SELECT DISTINCT
	t1.SCENARIO
FROM "TRSDB"."V_TRS_CFC_10Y_DEDUCTION_RESULT" t1
...
SELECT distinct t1.*,t2.current_code as enetity_current_code,
	...
	EXTRACT(Year FROM YYYY) AS OCC_YEAR,
	EXTRACT(Year FROM YYYY) + 10 AS EXP_YEAR,
	EXTRACT(Year FROM YYYY) + 10 - EXTRACT(Year FROM period) AS OFFSET_YEAR,
	CASE WHEN EXTRACT(Year FROM period) > EXTRACT(Year FROM YYYY) + 10 THEN 'Y' 
...
nvl(t1.TOTAL_LOSS,0) - nvl(t1.PAST_DEDUCTION,0) - nvl(t1.THIS_DEDUCTION,0) AS NO_DEDUCTED_AMOUNT,
(t1.TOTAL_LOSS- (nvl(t1.TOTAL_LOSS,0) - nvl(t1.PAST_DEDUCTION,0) - nvl(t1.THIS_DEDUCTION,0)))/t1.TOTAL_LOSS AS DEDUCTED_PERCENTAGE
from v t1
...
<UUID uuid="216af939-1f3a-46b0-9cfa-af8e70db5ce4"/>
...
<TemplateCloudInfoAttrMark class="com.fr.plugin.cloud.analytics.attr.TemplateInfoAttrMark" pluginID="com.fr.plugin.cloud.analytics.v11" plugin-version="3.26.0.20240327">
